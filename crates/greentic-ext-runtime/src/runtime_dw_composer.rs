...
//!
//! Composer extensions export `greentic:dw-composer/composer@0.1.0` instead
//! of the generic `greentic:extension-design/tools` interface.  This module
//! provides type-safe invoke methods that resolve the composer interface
//! directly from the component, bypassing the `invoke_tool` / `tools` path
//! that raises boot warnings for every composer extension.
//!
...
/// Fully-qualified interface name as wasmtime resolves it from the component.
const COMPOSER_IFACE_NAME: &str = "greentic:dw-composer/composer@0.1.0";
...
impl ExtensionRuntime {
    ...
    /// the rendered manifest JSON on success or an error string on failure.
    ///
    ...
    /// the rendered manifest JSON; the `Err` variant carries a human-readable
    /// error message the designer can surface to the user.
    ///
    /// # Errors
    ///
    /// Returns `RuntimeError::NotFound` when no extension is loaded at
    ...
    /// inside the composer is NOT a `RuntimeError` — it is the `Err(String)`
    /// inside the returned `Ok(Result<String, String>)`.
...
/// Resolve `greentic:dw-composer/composer@0.1.0` from a wasmtime component
/// instance.
///
/// Returns the `ComponentExportIndex` for the interface and the resolved name
/// (for use in subsequent error messages). Only one version exists for this
/// interface, so there is no version-fallback chain like `resolve_design_iface`.
///
/// # Errors
///
/// Returns `RuntimeError::Wasmtime` with a descriptive message when the
/// component does not export the expected interface.
fn resolve_dw_composer_iface(
    ...
    instance
        .get_export_index(&mut *store, None, COMPOSER_IFACE_NAME)
        .map(|idx| (idx, COMPOSER_IFACE_NAME.to_string()))
        .ok_or_else(|| {
            RuntimeError::Wasmtime(anyhow::anyhow!(
                "extension does not export interface '{COMPOSER_IFACE_NAME}'"
            ))
        })
}
